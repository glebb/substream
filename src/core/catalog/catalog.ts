...
import type { VodCatalog, VodCatalogItem, VodContentType, VodSearchOptions } from "./types.ts";
...
export function createVodCatalogItem(entry: M3uEntry, addedAt = Date.now()): VodCatalogItem | null {
  if (classifyM3uEntry(entry).kind !== "vod") return null;
...
export function buildVodCatalog(entries: Iterable<M3uEntry>): VodCatalog {
  ...
  return { items, groupCounts, contentTypeCounts };
}
